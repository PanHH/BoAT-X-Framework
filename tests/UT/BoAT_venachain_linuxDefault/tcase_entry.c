...
char g_venachain_private_key_buf[1024];
...
{       
    ...
    if (TEST_KEY_TYPE == "BOAT_KEYPAIR_PRIKEY_FORMAT_NATIVE")
    {
        fd = open("../../../tests/BoAT_venachain_linuxDefault/pri_key/native.key", O_RDONLY);
        if (fd < 0)
        {
            return -1;
        }
    ...
    {
        fd = open("../../../tests/BoAT_venachain_linuxDefault/pri_key/pkcs_key.key", O_RDONLY);
        if (fd < 0)
        {
            return -1;
        }
